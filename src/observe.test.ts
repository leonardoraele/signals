...
import { createObservedProxy, isObservedProxy, unwrapObservedProxy } from './observe';
import { Computed } from './computed';

describe('observed proxy', () => {
	it('should create an observed proxy', () => {
		const obj = { a: 1 };
		const proxy = createObservedProxy(obj);
		expect(isObservedProxy(proxy)).toBe(true);
	...
	it('should unwrap observed proxy', () => {
		const obj = { a: 1 } as { a?: number };
		const proxy = createObservedProxy(obj);
		const doubleA = new Computed(() => 'a' in proxy ? proxy.a * 2 : 0);
		expect(doubleA.value).toBe(2);
		expect(doubleA.dirty).toBe(false);
		const unwrapped = unwrapObservedProxy(proxy);
		unwrapped.a = 3;
		delete unwrapped.a;
		expect(doubleA.dirty).toBe(false);
		expect(doubleA.value).toBe(2);
	...
	it('should not observe nested objects when deep option is disabled', () => {
		const obj = { a: { b: 2 } };
		const proxy = createObservedProxy(obj, { deep: false });
		const doubleAB = new Computed(() => proxy.a.b * 2);
		expect(doubleAB.value).toBe(4);
		expect(isObservedProxy(proxy.a)).toBe(false);
		proxy.a.b = 3;
	...
	it('should handle nested objects with deep option', () => {
		const obj = { a: { b: 2 } };
		const proxy = createObservedProxy(obj, { deep: true });
		const doubleAB = new Computed(() => proxy.a.b * 2);
		expect(isObservedProxy(proxy.a)).toBe(true);
		expect(doubleAB.value).toBe(4);
		proxy.a.b = 3;
	...
	it('should observe arrays', () => {
		const arr = [0, 1, 2, 3, 4];
		const proxy = createObservedProxy(arr);
